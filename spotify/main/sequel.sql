...
create view master_sp.song_cont as
select 
distinct(left(date_added, 4)) as "year"
,count(track_list) as song_ct
from master_sp.dim_all_details
...
from master_sp.dim_all_details group by date_added, track_list) as temp
...
from master_sp.dim_all_details group by date_added, artists_list) as temp
group by rn order by rn
